...
int motor1StartTime = 0, motor2StartTime = 0, motor3StartTime = 0, motor4StartTime = 0; 
int motor1Period, motor2Period, motor3Period, motor4Period;
...
void setup () {
  ...
  motorServo[0].attach (7);  // attaches the servo on pin 9 to the servo object 
  motorServo[1].attach (8);
  motorServo[2].attach (9);
  motorServo[3].attach (10);
  
  motorServo[2].write (0);
  motorServo[3].write (0);
...
//    motorServo[channel].write(0);
  ...
  for (int channel = 0; channel < 4; channel++) { 
    ...
    // change the resistance on this channel from min to max:
        for (int level = 100; level < 128; level++) {
