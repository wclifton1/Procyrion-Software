...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const int offServoPulseWidth = 800;
  const int minServoPulseWidth = 1300;
  const int maxServoPulseWidth = 2000;
  const int servoPulseWidthKeyboardDelta = 100;
  ...
  int servoPulseWidth = 1500, oldServoPulseWidth = 1500;            // initial servo pulse width in 10s of usec; motor starts running at about 70 and maxes out at 120 or so 

  const float proportionalGain = 0.5;    // 0.65
  const float integralGain = 0.00;         // 0.1
  const float integrationInterval = 2.0;  // 2.0
  const int cyclesToAverage = 15;  // put back to 10
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  void loop () {
    ...
    if (!wantedOn)            
      myservo.writeMicroseconds (offServoPulseWidth);      // set throttle to zero if desired state is off
    else
    ...
    if (restartAttempts <= restartAttemptsLimit)
      if (restartInProgress) {
        if ((int) (millis () - restartStartTime) > startDelay) {
          ...
          myservo.writeMicroseconds (servoPulseWidth);      // set throttle to last desired setting
          delay (25);
          restartInProgress = false;
        }
      }
      else {
        myservo.writeMicroseconds (offServoPulseWidth);                    // set throttle to zero
...
//      Serial.print (wantedOn);
      ...
      Serial.print (";  RPMPerTick: ");
      Serial.println (RPMPerTick);
